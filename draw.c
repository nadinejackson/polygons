...
void draw_polygons( struct matrix *polygons, screen s, color c )
{
  ...
  for(poly = 0; poly < polygons->lastcol - 2; poly += 3)
    {
      
      normal = calculate_normal(polygons, poly);
      
      if(normal[2] > 0)
	{
	  //draw lines
	  draw_line(polygons->m[0][poly],
		    polygons->m[1][poly],
		    polygons->m[0][poly + 1],
		    polygons->m[1][poly + 1],
		    s, c);
...
void add_box( struct matrix * edges,
  ...
  add_polygon(edges, x1, y1, z1, x1, y1, z0, x0, y1, z1);
